...
def mirror_aero_data_axisymmetric(df_input: pd.DataFrame) -> pd.DataFrame:
    ...
    for orig_row_dict in original_records:
        ...
        if not np.isclose(beta_orig, 0.0):
            ...
            if cs_col in mod_row and not pd.isna(orig_rowDict[cs_col]):
        ...
        if not np.isclose(aoa_orig, 0.0) and not np.isclose(beta_orig, 0.0):
            ...
            if cfz_col in mod_row: mod_row[cfz_col] = -orig_rowDict[cfz_col]
   ...
   # --- Prepare data for specific Beta=0, AoA 0-25 deg plots ---
    ...
    if all(c in df_beta0_proc_plot_range.columns for c in [cl_col_name, 'CL_lower', 'CL_upper', cd_modified_col_name]):
        if not df_beta0_proc_plot_range.empty:
            ...
            # --- Add max L/D point ---
            if 'CL' in df_beta0_proc_plot_range.columns and 'CDtot_modified' in df_beta0_proc_plot_range.columns:
                ld = df_beta0_proc_plot_range['CL'] / df_beta0_proc_plot_range['CDtot_modified']
                max_ld_idx = ld.idxmax()
                plt.scatter(df_beta0_proc_plot_range.loc[max_ld_idx, 'CDtot_modified'],
                            df_beta0_proc_plot_range.loc[max_ld_idx, 'CL'],
                            color='red', zorder=5, label='Max L/D')
                plt.annotate(f"Max L/D\n(CL={df_beta0_proc_plot_range.loc[max_ld_idx, 'CL']:.2f}, CD={df_beta0_proc_plot_range.loc[max_ld_idx, 'CDtot_modified']:.3f})\nL/D={ld[max_ld_idx]:.1f}",
                             (df_beta0_proc_plot_range.loc[max_ld_idx, 'CDtot_modified'], df_beta0_proc_plot_range.loc[max_ld_idx, 'CL']),
                             textcoords="offset points", xytext=(30,10), ha='left', color='red')

            # --- Plot cruise L/D point at AoA=6 ---
            cruise_aoa = 6.0
            if cruise_aoa is not None and 'AoA' in df_beta0_proc_plot_range.columns:
                cruise_row = df_beta0_proc_plot_range.iloc[(df_beta0_proc_plot_range['AoA']-cruise_aoa).abs().argsort()[:1]]
                plt.scatter(cruise_row['CDtot_modified'], cruise_row['CL'], color='green', zorder=5, label='Cruise L/D')
                plt.annotate(f"Cruise\n(AoA={cruise_row['AoA'].values[0]:.1f})\nL/D={float(cruise_row['CL']/cruise_row['CDtot_modified']):.1f}",
                             (cruise_row['CDtot_modified'].values[0], cruise_row['CL'].values[0]),
                             textcoords="offset points", xytext=(-80,-20), ha='right', color='green')
